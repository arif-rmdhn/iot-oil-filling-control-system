...
const int num_index = 4, relay_pump = 23;
float duration_us, distance_cm, data_prev_sen, data_prev_SP, get_average_data[num_index], data1 = 0, data2 = 0, realData1, realData2;
int readIndex = 0;
int set_point_high = 500, set_point_low = 0, condition = 1;
char data_Keypad;
int status_motor = 0, interupsi = 1;
String get_keyPad = "";
...
void read_sensor() {
  ...
  realData2 = realData1 * 9.3 * 9.3;  // Konversi ml
...
void cek_kondisi() {
  ...
  pump_stat(status_motor);
}

//------------------------------------------------------- FUNCTION CEK LEVEL 1 --------------------------------------------------//
void cek_level_1() {
  ...
  digitalWrite(relay_pump, status_motor);
  lcd.setCursor(0, 1);
  lcd.print("D:Set|SP:" + String(set_point_high) + "ml");
}

//------------------------------------------------------- FUNCTION CEK LEVEL 2 --------------------------------------------------//
void cek_level_2() {
  if (!interupsi) {
    if (data_prev_sen <= set_point_low && !status_motor) {
      status_motor = 1;
    } else if (data_prev_sen >= set_point_high && status_motor) {
      status_motor = 0;
    }
  } else {
    status_motor = 0;
      ...
      if (data_Keypad) {
        ...
        if (data_Keypad == '#') {
          if (get_keyPad.length() > 0) {
            ...
            Serial.println("Tidak ada data yang dimasukkan!");
            break;
          }
        ...
        // DELETE
          ...
          lcd.clear();
          Serial.println("Hanya angka yang diterima!");
          lcd.setCursor(3, 0);
